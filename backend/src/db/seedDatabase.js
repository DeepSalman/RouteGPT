...
async function upsertBus(client, bus) {
  ...
  if (bus.slug) {
    const result = await client.query(
      `
        INSERT INTO buses (
          name, name_bn, slug, source_url, seating_type, fare_range,
          operator, start_time, end_time, ticketing_system
        )
  ...
  const result = await client.query(
    `
      INSERT INTO buses (
        name, name_bn, seating_type, fare_range, operator,
        start_time, end_time, ticketing_system
      )
...
async function main() {
  ...
  console.log(
    `Seeded ${result.buses} buses, ${result.stops} ordered stops, and ${result.aliases} aliases.`
  );
}
